...
public class ToBinaryUnit extends AutoEncoder {
    public ToBinaryUnit(int inW, int inH, int inD, int oD) {
        super(inW, inH, inD, oD);
        if (inW!=1 || inH!=1) {
            throw new Error(
                    "Binary units cannot be added to convolved layers."
            );
        }
        if (oD!=inD) {
            throw new Error(
                    "Binary units require same input and output lengths."
            );
        }
    }
    ...
    public void encode() {
        ...
        for (int i = 0; i < inputLength; i++) {
            getOutputArray()[i] = (input[i] > 0) ? 1 : 0;
        }
    }
