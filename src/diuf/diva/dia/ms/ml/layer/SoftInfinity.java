 ...
 */
public class SoftInfinity extends AbstractLayer {
     ...
     */
    public SoftInfinity(float[] inputArr, int outputSize) {
     ...
     * @param outputSize the number of neurons
     */
    public SoftInfinity(float[] inputArr, int inputSize, int outputSize) {
     ...
     */
    public SoftInfinity(float[] inputArray, int inputSize, int outputSize, float[][] weight, float[] bias) {
     ...
     */
    SoftInfinity(DataInputStream is) throws IOException {
        super(is);
    }
     ...
     */
    SoftInfinity(String fname) throws IOException {
        super(fname);
    }
    ...
    public void compute() {
        for (int o = 0; o < outputSize; o++) {
            ...
            if (wSum[o]<0) {
                // derivative: 1 / (wSum[o]+1)^2
                output[o] = wSum[o] / (1.0f + wSum[o]);
            } else {
                // derivative: 1 / (wSum[o]-1)^2
                output[o] = -wSum[o] / (wSum[o] - 1.0f);
            }
        }
    }
    ...
    public float backPropagate() {
        ...
        if (prevErr==null) {
            for (int o = 0; o < outputSize; o++) {
                ...
                float bot = (wSum[o]<0) ? wSum[o]+1 : wSum[o]-1;
