 ...
 */
public class SoftPlusLayer extends NeuralLayer {
     ...
     */
    public SoftPlusLayer(float[] inputArr, int outputSize) {
        super(inputArr, inputArr.length, outputSize);
    }
     ...
     * @param outputSize the number of neurons
     */
    public SoftPlusLayer(float[] inputArr, int inputSize, int outputSize) {
        super(inputArr, inputSize, outputSize, null, null);
    }
     ...
     */
    public SoftPlusLayer(float[] inputArray, int inputSize, int outputSize, float[][] weight, float[] bias) {
     ...
     */
    SoftPlusLayer(DataInputStream is) throws IOException {
        super(is);
    }
     ...
     */
    SoftPlusLayer(String fname) throws IOException {
        super(fname);
    }
    ...
    public void compute() {
        for (int o = 0; o < outputSize; o++) {
            ...
            output[o] = (float)Math.log(1.0f+(float)Math.exp(wSum[o]));
        }
    }
    ...
    public float backPropagate() {
        ...
        if (prevErr==null) {
            for (int o = 0; o < outputSize; o++) {
                ...
                float fact = 1.0f / (1.0f+(float)Math.exp(-wSum[o])) * err[o];
                for (int i = 0; i < inputSize; i++) {
