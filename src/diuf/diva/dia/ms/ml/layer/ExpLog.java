...
/**
 * This layer is experimental and has NOT been tested. Its function is
 * y = log(exp(sum w_i * e^x_i))
 * @author Mathias Seuret, Michele Alberti
 */
public class ExpLog extends AbstractLayer {
     ...
     */
    public ExpLog(float[] inputArr, int outputSize) {
     ...
     * @param outputSize the number of neurons
     */
    public ExpLog(float[] inputArr, int inputSize, int outputSize) {
     ...
     */
    public ExpLog(float[] inputArray, int inputSize, int outputSize, float[][] weight, float[] bias) {
     ...
     */
    ExpLog(DataInputStream is) throws IOException {
        super(is);
    }
     ...
     */
    ExpLog(String fname) throws IOException {
        super(fname);
    }
    ...
    public float backPropagate() {
        ...
        if (prevErr==null) {
            for (int o = 0; o < outputSize; o++) {
                ...
                for (int i = 0; i < inputSize; i++) {
                    float fact = 1 / wSum[o] * exp[i][o] * err[o];
